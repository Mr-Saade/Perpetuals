...
contract MinimumPerps is ERC4626, Ownable2Step {
    ...
    function _setBorrowingPerSharePerSecond(
        uint256 _borrowingPerSharePerSecond
    ) internal {
        if (_borrowingPerSharePerSecond > MAX_BORROWING_RATE) {
            revert Errors.InvalidBorrowingFeeRate(_borrowingPerSharePerSecond);
        }
    ...
    function getNetPnl(
        ...
        if (isLong) {
            pnl =
                int256(openInterestTokensLong * indexPrice) -
                int256(openInterestLong);
        } else {
            pnl =
                int256(openInterestShort) -
                int256(openInterestTokensShort * indexPrice);
        }
    }
    ...
    function totalAssets() public view override returns (uint256) {
        ...
        int256 netTraderPnlInCollateral = (traderPnlLong + traderPnlShort) /
            getCollateralPrice().toInt256();

        if (netTraderPnlInCollateral > 0) {
            if (netTraderPnlInCollateral.toUint256() > _totalDeposits) {
                revert Errors.TraderPnlExceedsDeposits();
            }
    ...
    /**
     * @notice Creates / Increases the position size for a trader.
     * @dev Handles leverage and collateral adjustments.
     * @param isLong True if opening a long position, false for a short position.
     * @param sizeDeltaUsd The amount by which to increase the position size (in USD).
     * @param collateralDelta The amount of collateral to create the position / the additional collateral being added when incresing and exising position.
     */
    ...
    function increasePosition(
        ...
        mapping(address => Position) storage positions = isLong
            ? longPositions
            : shortPositions;
        ...
        if (position.collateralAmount == 0 && position.sizeInUsd == 0) {
            newCollateral = collateralDelta;
            newSize = sizeDeltaUsd;
        } else {
            newCollateral = position.collateralAmount + collateralDelta;
            newSize = position.sizeInUsd + sizeDeltaUsd;
        }

        uint256 leverage = newSize / (newCollateral / 1e6);

        if (leverage > maxLeverage) {
            revert Errors.MaxLeverageExceeded(leverage, maxLeverage);
        }

        uint256 indexTokenPrice = getIndexPrice();
        uint256 indexTokenDelta = isLong
            ? sizeDeltaUsd / indexTokenPrice
            : Math.ceilDiv(sizeDeltaUsd, indexTokenPrice);
    ...
    /**
     * @notice Closes / decreases the position size for a trader.
     * @param isLong True if opening a long position, false for a short position.
     * @param sizeDeltaUsd The amount by which to decrease the position size (in USD).
     * @param collateralDelta The amount of collateral, trader is wlling to withdraw when decreasing or closing the position.
     */
    ...
    function decreasePosition(
        ...
        if (isPositionLiquidatable(msg.sender, isLong)) {
            revert Errors.PositionIsLiquidatable();
        }
    }
    ...
    function isPositionLiquidatable(
        ...
        Position memory position = isLong
            ? longPositions[trader]
            : shortPositions[trader];
        ...
        // Account for PnL
        (int256 positionPnl, ) = _calculateRealizedPnl(
            position,
            isLong,
            position.sizeInUsd
        );
    ...
    function liquidate(address trader, bool isLong) external {
        if (!isPositionLiquidatable(trader, isLong)) {
            revert Errors.PositionNotLiquidatable();
        }
        mapping(address => Position) storage positions = isLong
            ? longPositions
            : shortPositions;
    ...
    function _decreasePosition(
        ...
        (int256 realizedPnl, uint256 sizeDeltaTokens) = _calculateRealizedPnl(
            position,
            isLong,
            sizeDeltaUsd
        );
        ...
        if (pendingBorrowingFees > position.collateralAmount && isLiquidation) {
            // Allow liquidations to insolvently close
            pendingBorrowingFees = position.collateralAmount;
            position.collateralAmount = 0;
            totalDeposits += pendingBorrowingFees;
        } else {
            if (position.collateralAmount < pendingBorrowingFees) {
                revert Errors.InsufficientCollateralForBorrowingFees(
        ...
        //According to this design choice, i prioritize liquidation fees for liquidators than the Lp payouts

        if (isLiquidation) {
            uint256 liquidationFeeAmount = (position.collateralAmount *
                liquidationFeeBp) / BASIS_POINTS;
            position.collateralAmount -= liquidationFeeAmount;
            IERC20(asset()).safeTransfer(msg.sender, liquidationFeeAmount);
        }

        if (realizedPnl < 0) {
            uint256 positionLossInCollateral = realizedPnl.abs() /
                collateralTokenPrice;
            if (
                isLiquidation &&
                position.collateralAmount < positionLossInCollateral
            ) {
                // Allow insolvent liquidations
                positionLossInCollateral = position.collateralAmount;
                position.collateralAmount = 0;
            } else if (position.collateralAmount < positionLossInCollateral) {
                revert Errors.InsufficientCollateralForLoss(
                    positionLossInCollateral,
                    position.collateralAmount
                );
            }
            position.collateralAmount -= positionLossInCollateral;
            totalDeposits += positionLossInCollateral;
        }
        ...
        if (outputAmount > 0) {
            IERC20(asset()).safeTransfer(trader, outputAmount);
        }
    }
    ...
    function _calculateRealizedPnl(
        ...
        bool isLong,
        uint256 sizeDeltaUsd
    ) internal view returns (int256 realizedPnl, uint256 sizeDeltaTokens) {
        int256 currentPositionValue = (position.sizeInTokens * getIndexPrice())
            .toInt256();
        int256 totalPnl = isLong
            ? currentPositionValue - position.sizeInUsd.toInt256()
            : position.sizeInUsd.toInt256() - currentPositionValue;

        realizedPnl =
            (totalPnl * sizeDeltaUsd.toInt256()) /
            position.sizeInUsd.toInt256();
        if (position.sizeInUsd == sizeDeltaUsd) {
            sizeDeltaTokens = position.sizeInTokens;
        } else if (isLong) {
            sizeDeltaTokens = Calc.roundUpDivision(
    ...
    function getPendingBorrowingFees(
        address trader,
        bool isLong
    ...
    function _calculateBorrowingFees(
        Position memory position
    ) internal view returns (uint256 pendingBorrowingFees) {
        uint256 pendingBorrowingFeesUsd = (position.sizeInUsd *
            (block.timestamp - position.lastUpdatedAt) *
            borrowingPerSharePerSecond) / 1e30;
    ...
    function _validateNonEmptyPosition(Position memory position) internal pure {
        if (
    ...
    function _validateMaxUtilization() internal view {
        uint256 indexTokenPrice = getIndexPrice();
        uint256 collateralTokenPrice = getCollateralPrice();
        ...
        uint256 totalReserved = reservedForLongs + reservedForShorts; //total OI

        uint256 valueOfDeposits = totalDeposits * collateralTokenPrice;
        uint256 maxUtilizableDeposits = (valueOfDeposits *
            maxUtilizationRatio) / PRECISION;

        if (totalReserved > maxUtilizableDeposits) {
            revert Errors.MaxUtilizationBreached(
                maxUtilizableDeposits,
                totalReserved
            );
        }
    }
